...
**MANDATORY**: Use AskUserQuestion to let the user choose what to do — this is the one place AskUserQuestion is allowed, because it's a menu choice with no correct answer, not a quiz. Analyze the dashboard to build context-aware options, then present them.
...
**CRITICAL**: Read `references/quiz-rules.md` before crafting ANY question. All questions are open-response — never multiple choice.

### Phase 4: Present Quiz

Post the 4 questions as plain text in your message. The user answers each one **in their own words** — there are no answer options to choose from. Never use AskUserQuestion for quiz questions.

- Number them Q1–Q4, each on its own line
- Phrase each so it can only be answered by someone who knows the material — no hints in the wording
- Ask for the reasoning where it matters ("…and why?")

Wait for the user's typed answers before grading.

### Phase 5: Grade & Explain

1. Show results table (question / correct answer / user's answer / ✓ / ✗ / partial)
2. Wrong or partial answers: concise explanation that names the misconception behind what they wrote, not just the right answer
...
- Update proficiency badges from the struggle rate (Wrong ÷ attempts): 🟦 0-10% · 🟩 11-30% · 🟨 31-60% · 🟥 61-100% · ⬜ no data
- Update stats: total questions, cumulative struggle, unresolved/resolved counts, weakest/strongest
...
- All quiz questions are open-response — the user types their own answer, NEVER multiple choice
- NEVER use AskUserQuestion for quiz questions (only for the Phase 2 session-type menu)
- NEVER include hints in the question wording
